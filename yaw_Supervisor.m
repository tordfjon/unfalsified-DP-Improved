...
function [sys,x0,str,ts] = yaw_Supervisor(t,x,u,flag, ...
...
elseif flag == 2 
   ...
   if u_dat.m == 0               %% If all controllers has been falsified
         error('All Yaw Controllers Has been falsified; try increasing the set of initial candidate controllers');
...
elseif flag == 9
   sys = [];  
   disp('Number of unfalsified yaw-controllers at the end = ')
   disp(u_dat.m)
end