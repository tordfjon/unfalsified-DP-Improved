...
yaw_demux_1 = char(find_system(gcb,'LookUnderMasks','all','Tag','yaw_demux_1'));
yaw_demux_2 = char(find_system(gcb,'LookUnderMasks','all','Tag','yaw_demux_2'));
yaw_mux_1 = char(find_system(gcb,'LookUnderMasks','all','Tag','yaw_mux_1'));
yaw_mux_2 = char(find_system(gcb,'LookUnderMasks','all','Tag','yaw_mux_2'));
...
total_old = str2num(get_param(yaw_demux_1,'Outputs'));
...
if (total_old ~= total_new)
    ...
    yaw_per_monitor =  char(find_system(gcb,'LookUnderMasks','all','Tag','yaw_per_monitor'));
      ...
      set_param(yaw_demux_1,'Outputs',total_new_char); %% setting new port dimensions
      set_param(yaw_demux_2,'Outputs',total_new_char);
      set_param(yaw_mux_1,'Inputs',total_new_char);
      set_param(yaw_mux_2,'Inputs',total_new_char);
      ...
      for(i=1:total_new)
          ...
          add_line(yaw_per_monitor,d_2,m_2);
      end
end