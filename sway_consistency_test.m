...
function [J,J1,J2,J3, xfw1, xfw2] = sway_consistency_test(aw1, bw1, cw1, dw1, xw1, ...
                                            aw2, bw2, cw2, dw2, xw2, ...
                                            u, y, r, ...
...
temp = j1^2 + j2^2 -j3^2 - (1/50);% - 2.1213^2; % Including std. of the time varying environmental force. Tord: This is the evaluated cost function according to (9) in the paper. See notes in OneNote

%% Writing temporary values to global var
sway_u_cost.temp(index,count) = temp;
sway_u_cost.temp1(index,count) = j1;
sway_u_cost.temp2(index,count) = j2;
sway_u_cost.temp3(index,count) = j3;
...
if count > count_limit
    J = sum(fliplr(sway_u_cost.temp(index,count-count_limit:count)).*exp(-forgetting_rate*t)).*delta;
else 
    t_hor = (count-1)*delta;
    J = sum(fliplr(sway_u_cost.temp(index,1:count)).*exp(-forgetting_rate*(0:delta:t_hor))).*delta;
end
...
sway_u_cost.myj1(index,count) = J1;
sway_u_cost.myj2(index,count) = J2;
sway_u_cost.myj3(index,count) = J3;
sway_u_cost.myj(index,count) = J; %Tord: This is the data storage for the cost function values of all controllers
