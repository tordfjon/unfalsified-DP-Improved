...
sway_demux_1 = char(find_system(gcb,'LookUnderMasks','all','Tag','sway_demux_1'));
sway_demux_2 = char(find_system(gcb,'LookUnderMasks','all','Tag','sway_demux_2'));
sway_mux_1 = char(find_system(gcb,'LookUnderMasks','all','Tag','sway_mux_1'));
sway_mux_2 = char(find_system(gcb,'LookUnderMasks','all','Tag','sway_mux_2'));
...
total_old = str2num(get_param(sway_demux_1,'Outputs'));


if (total_old ~= total_new)
    sway_unfalsification  = char(find_system(gcb,'LookUnderMasks','all','Tag','sway_unfalsification'));  
    sway_per_monitor    = char(find_system(gcb,'LookUnderMasks','all','Tag','sway_per_monitor'));
      %% deleting existing lines between mux & demux blocks      
      for(i=1:total_old)      
          d_1 = sprintf('%s/%d',get_param(sway_demux_1,'Name'),i);
          d_2 = sprintf('%s/%d',get_param(sway_demux_2,'Name'),i);
          m_1 = sprintf('%s/%d',get_param(sway_mux_1,'Name'),i);
          m_2 = sprintf('%s/%d',get_param(sway_mux_2,'Name'),i);
          delete_line(sway_unfalsification,d_1,m_1);
          delete_line(sway_per_monitor,d_2,m_2);
      end

      %% Setting the port size of mux & demux blocks to the number of initial candidate controllers
      total_new_char = num2str(total_new);
      set_param(sway_demux_1,'Outputs',total_new_char); %% setting new port dimensions
      set_param(sway_demux_2,'Outputs',total_new_char);
      set_param(sway_mux_1,'Inputs',total_new_char);
      set_param(sway_mux_2,'Inputs',total_new_char);
      ...
      for(i=1:total_new)
          ...
          add_line(sway_per_monitor,d_2,m_2);
      end
end